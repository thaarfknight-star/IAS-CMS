...
# پورت‌های رایج جایگزین برای RTSP (علاوه بر 554 پیش‌فرض) که دوربین‌ها/NVRهای
# ارزان‌قیمت و ژنریک به‌جای پورت استاندارد از آن‌ها استفاده می‌کنند.
# رفع باگ «بعد از پیدا/افزودن NVR، کانال‌ها/دوربین‌های متصل به آن پیدا نمی‌شوند»:
# این ثابت قبلاً فقط داخل device_detect.py (برای تشخیص نوع دستگاه) تعریف و
# استفاده می‌شد؛ NVRScanThread (نوار_scanner.py) که کانال‌های واقعی را بعد از
# افزودن/پیدا کردن NVR پیدا می‌کند، فقط دقیقاً همان یک پورتی را امتحان
# می‌کرد که در فرم افزودن NVR وارد شده بود (پیش‌فرض 554). روی هر NVR ای که
# RTSP را روی پورتی غیر از 554 اجرا می‌کرد (خیلی از NVR/DVRهای ژنریک)، این
# باعث می‌شد اسکن کانال‌ها همیشه با «هیچ کانالی یافت نشد» شکست بخورد، حتی
# با اینکه IP/یوزرنیم/پسورد کاملاً درست بودند. حالا این ثابت مشترک در هر دو
# فایل استفاده می‌شود تا هر دو مرحله (تشخیص نوع دستگاه و اسکن کانال‌های NVR)
# همان مجموعه پورت‌های جایگزین را امتحان کنند.
